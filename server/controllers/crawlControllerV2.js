...
const CSVCrawlLinkv2 = asyncHandler(async (req, res) => {
      ...
      setTimeout(function () {
        ...
        fetch(newLinkCrawled, {
          ...
          .then((response) => {
            ...
            const linkInDB = DBLINK.findOne({ urlTo: linkCrawled.urlTo });
            if (!linkInDB) {
              createLink({
                urlFrom: linkCrawled.URLFrom,
                urlTo: linkCrawled,
            ...
            setTimeout(async function () {
              fetch(newLinkCrawled, {
                ...
                .then((response) => {
                  ...
                  const linkInDB = DBLINK.findOne({
                    urlTo: newLinkCrawled.urlTo,
                  });
                  if (!linkInDB) {
                    createLink({
                      urlFrom: linkCrawled.URLFrom,
                    ...
                    console.log("done creating", newLinkCrawled);
                  } else {
                    DBLINK.findOneAndUpdate(
                      { urlTo: newLinkCrawled.urlTo },
                      { $set: { dateLastChecked: format } },
                    ...
                    console.log("done updating", newLinkCrawled);
                  }
                  index++;
                  if (array.length - 1 === index) {
                    const endTime = performance.now();
                ...
                .catch((error) => {
                  ...
                  let linkObj = {
                    URLFrom: newLinkCrawled.URLFrom,
                    urlTo: newLinkCrawled,
                    text: newLinkCrawled.text,
                    linkStatus: "Error on this link",
                    statusText: "Error on this link",
                    linkFollow: newLinkCrawled.linkFollow,
                  };
                  const linkInDB = DBLINK.findOne({
                    urlTo: newLinkCrawled.urlTo,
                  });
                  if (!linkInDB) {
                    ...
                    DBLINK.findOneAndUpdate(
                      { urlTo: newLinkCrawled.urlTo },
                      { $set: { statusText: "Error on this link" } },
                      { $set: { linkStatus: "Error on this link" } },
                      { $set: { dateLastChecked: format } },
                      { runValidators: true, new: true }
                    );
                    console.log("done updating", newLinkCrawled);
